...
class TelegramClient:
    def __init__(self,
        ...
        self._set_handlers(
            lambda update, _: on_text(update.effective_message.text, update.message.reply_text),
            lambda update, _: on_reply(update.effective_message.text, update.message.reply_text)
        )
    ...
    def _set_handlers(self, on_text, on_reply):
        ...
        dispatcher.add_handler(CommandHandler('start', lambda update, _: update.message.reply_text("Hi!")))
        dispatcher.add_handler(MessageHandler(Filters.reply, on_reply))
        dispatcher.add_handler(MessageHandler(Filters.text, on_text))
        dispatcher.add_error_handler(lambda update, _, err: LOGGER.error('Update "%s" caused error "%s"', update, err))
